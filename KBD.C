...
#ifdef __DJGPP__
/* DJGPP version */
inline unsigned short kbd_get_ds(void) {
...
#ifdef __WATCOMC__
extern void (__interrupt __far kbd_handler)();
#else
...
__dpmi_paddr kbd_oldvect;
__dpmi_paddr kbd_newvect;
 
#else
/* WATCOM C++ version */
static void (__interrupt __far *kbd_oldvect)();
static void (__interrupt __far *kbd_newvect)();
#endif
...
int kbd_init(void)
{
	if (kbd_initialized) return 1;
        ...
        #ifdef __DJGPP__
        /* DJGPP version */
        kbd_newvect.offset32 = (unsigned long) &kbd_handler;
        kbd_newvect.selector = (unsigned short) _my_cs();
        __dpmi_get_protected_mode_interrupt_vector(9, &kbd_oldvect);
        __dpmi_set_protected_mode_interrupt_vector(9, &kbd_newvect);

		#else
        /* WATCOM C++ version */
        kbd_newvect = &kbd_handler;
        kbd_oldvect = _dos_getvect(9);
...
void kbd_uninit(void)
{
	if (!kbd_initialized) return;
        #ifdef __DJGPP__
        /* DJGPP version */
        __dpmi_set_protected_mode_interrupt_vector(9, &kbd_oldvect);
